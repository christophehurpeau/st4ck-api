...
import apiResponseKoaMiddleware from '../core/library/koa/middlewares/api-response';
import apiResponseKoaMiddlewareHelpers from '../core/library/koa/middlewares/api-response-helpers';
...
try {
  ...
  router.use(async (ctx, next) => {
    ...
    const token = ctx.query && ctx.query.hasOwnProperty(getKey) ? ctx.query[getKey] : '';
